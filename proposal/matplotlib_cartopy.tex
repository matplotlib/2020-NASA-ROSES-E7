...
\item Increase documentation and test coverage of unit-aware plotting
\item General maintenance of Matplotlib and cartopy
\end{enumerate}

With a primarily volunteer project there are some projects that are
difficult to get done, either because they require longer blocks of
dedicated effort than can easily be done by volunteers or are tedious.
While there is a lot of work that can be done on a volunteer
best-effort, the work that does not get done can be critical and
un-block further volunteer effort.  Refactoring how cartopy binds
to PROJ is a project that is too big to be readily accomplished by
volunteers for [TODO REASONS].  Similarly, documenting and
completing the test coverage on unit support is both bigger than
can be done by a volunteer, it is also tedious.
...
unit-naive, making it the users responsibility to track and convert
the units prior to any computation.  Failing to do so can be
catastrophic [TODO cite the Mars mission lost due to unit issues? Too
  soon?].  Matplotlib has the ability to support plotting unit-aware
data structures.  The initial development of this capability was
supported by NASA and it is currently used to support spaceflight
...
inconsistencies discovered in this process.

\begin{enumerate}
\item how unit dispatch works
\item how unit testing works
\item expect to find bugs that need fixing
\item level of effort (.2-.5 FTE for a year)
\item coordinate with Monte developers
\end{enumerate}
