...
representation from the rest of the library.  We are developing a data
access API such that each \texttt{Artist} will have a single
\texttt{DataSource} object responsible for holding the data. This
design will allow us to over come the issues with the current design
and open the door to a variety of new functionality. The current work
is expected to produce design documents and a proof of concept
implementation by the end of 2021.  Based on this preliminary work
\textbf{we propose refactoring Matplotlib to use the new data model in
  all \texttt{Artist}s}.  One of immediate benefit of this work will
be to drastically improve our handling of data with physical units
attached.
...
unify how we treat units across the entire library.
...
- tie to numpy's dtype work strongly
...
the NumPy project to overhaul how the array dtypes are imlemented (NEP
...
Matplotlib is a foundational library, while
