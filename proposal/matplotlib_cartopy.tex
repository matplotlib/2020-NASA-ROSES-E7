...
Matplotlib~\cite{Hunter:2007} is a comprehensive open source library
for creating static, animated, and interactive visualizations in
Python that provides the visualization for missions and projects
across all SMD divisions, including flagship missions like the Hubble
...
MetPy and CartoPy which are used for Earth Science, all of which use
...
Matplotlib is an established open source plotting
...
estimates based on downloads and web traffic on the documentation,
...
infrastructure to some degree, but this is hard to prove in practice.
...
use of Matplotlib.  However, a canonical reference for
Matplotlib~\cite{Hunter:2007} has over 12,750 citations overall and
over 4,500 citations in ADS
...
twice as many citations).  AstroPy~\cite{robitaille2013astropy} has
about 4,500 citations.
...
responsible for rendering itself to the backend.  Thus, by changing
...
\texttt{Artist}s will be agnostic to exactly how their underlying data
is stored.
...
is passed by the user Matplotlib looks up a converter and sets
...
depend on Matplotlib~\cite{gh_deps:2021} and a recent study of the
...
SMD community.  Soliciting for examples of Matplotlib in SMD research
on social media yielded a (non-exhaustive) list of examples of using
Matplotlib
\begin{itemize}[noitemsep]
\item
  to study
...
There is personnel risk that we have not yet identified the RSE who will
be a key individual in carry out this work.  We are optimistic that we will
...
qualified candidates, one of whom in currently employed via NumFOCUS as
...
Matplotlib general maintenance, community engagement.  This includes,
...
project roadmapping, and community governance.
...
The data pipeline refactor is a critical part of the library that touches most artists, so we require a comprehensive plan.  As part
of developing this plan we will reach out to upstream data-structure
...
machinery, such as \texttt{unyt} (\url{https://unyt.readthedocs.io/}),
...
compatible as possible in the API changes needed for the refactoring.  The variation
...
our process, at least one maintainer not supported on this proposal
will need to sign off on any changes before they are merged.  This
will ensure that dissemination of understanding of the proposed work
and community buy-in to the proposed work.  Finally, by merging
changes to the default branch continuously through out the period of
work the incremental improvements will be released as part of our
bi-yearly release cadence.
...
build, package, and distribute Matplotlib.  Extensive User facing
documentation, including install instructions and usage guides, is
built and hosted at \url{https://matplotlib.org}.
