...
us to take on bigger projects than can be implemented by volunteers
and to ensure that the critical day-to-day tasks required to keep the
project healthy are done.  We propose to split the effort on this
grant between a major development effort, overhauling our internal
data representation, and general library maintenance.


\subsubsection{Data Model Implementation}
...
representation from the rest of the library. Currently each
\texttt{Artist} subclass holds its data as several arrays as
properties.  Each \texttt{Artist} uses different names and nothing
ensures that the data, if updated at run time, is self-consistent
until we try to draw the figure.  We are developing a data access API
such that each \texttt{Artist} will have a single \texttt{DataSource}
object responsible for holding the data. This design will allow us to
over come the issues with the current design and open the door to a
variety of new functionality.  The current work is expected to produce
design documents and a proof of concept implementation by the end of
2021.  Based on this preliminary work \textbf{we propose refactoring
  Matplotlib to use the new data model in all \texttt{Artist}s}.  One
of immediate benefit of this work will be to drastically improve our
handling of data with physical units attached.
...
in Python and Matplotlib, if configured properly, can be passed them
for unit-aware plotting.  \textbf{This capability it is currently used to
support spaceflight operations by Monte, JPL's mission design and
navigation software system} and the initial development was supported
by NASA.
...
passed by the user the we look up the converter and Axis
configuration, default axis label, limits, and functions for locating
and formatting the ticks, to use.  Matplotlib registers converters for
datetime and string-categorical types by default and third-party
libraries, like \texttt{unyt} and \texttt{pint}, to register
themselves.  Using this machinery users can control what units the
data is plotting in, which may be different than the units it was
passed in as, and specify the limits in physical units.


% what is wrong

However, there are several problems with the current unit support. It
is under documented making it difficult for users to understand and
use the capability and for developers to extend.  Further, each
plotting method is independently responsible for handling the units.
Coupled with a lack of full test coverage of plotting data with units,
this has allowed, over time, inconsistencies and regressions to
collect in the code base.  These range from unit-full data simply not
working with some methods to subtle inconsistencies between plotting
methods.  To address this we propose to use unit-handling as a
motivating feature while doing the \texttt{DataSource} refactoring.
By implementing all of the unit logic against the \texttt{DataSource}
we will be able to share the code across all \texttt{Artist}s and
unify how we treat units across the library.
...
\item and manage discussions about proposed enhancements, features,
  and API changes.
\end{itemize}
%one of the next three paragraphs needs to go!
...
reported gobto us.  While this critical work can be done by
volunteers, however supported developers help ensure that things
...
open PRs 1300 open Issues due to this imbalance, There may be critical
bug reports or insightful feature requests among the issue backlog,
while among the PR backlog are useful contributions or bug fixes that
would improve the libraries for direct users and downstream packages.
The large backlog is discouraging for new and occasional contributors
and distracting for core developers.


For 10 months in 2020\footnote{funded via CZI} we had a full time RSE
working on Matplotlib who had a measurable impact on the rate of PR
review and the rate of issue resolution, and fixed a number of long
standing bugs.  In 2020 Matplotlib resolved 125-200 PRs and 75-125
Issues a month, however even at this rate we only reduced the PR
backlog by 70 open PRs and held even on the Issue backlog.  We have
demonstrated supporting maintenance work has a positive effect and
\textbf{the additional resources requested here will help to reduce,
  but not eliminate, the maintenance backlog further}.
...
it has served us well, but it does not reflect recent developments in
software design, data structures, and visualization.  Refactoring the
data access layer, effectively splitting the \texttt{Artist} classes
in half, towards continuing to support and enable science for the next
20 years.  By separating the data storage and access logic form the
logic that maps the data to visual elements we unlock
\begin{itemize}[noitemsep]
  \item uniform handling of physical units,
...
We aim to take advantage of these developments to improve the
consistency, composability, and discoverabilty of the API.  The
ultimate goal is facilitating rich interactive domain-specific
visualizations.


For units to be as useful, we need all the components of the analysis
and visualization need to be unit aware.  There is on-going work by
the NumPy project to overhaul how the array dtypes are implemented (NEP
40, NEP 41, NEP 42, NEP 43).  One of the motivations of this work is
the ability to enable user defined parametric dtypes to support
tracking physical units and categorical data as a full-supported
feature of NumPy.  This work on the unit machinery in Matplotlib will
ensure that Matplotlib are ready to support the new numpy dtypes as
...
- tie to numpy/panda/xarray dtype work strongly
...
This documentation will be aimed at Matplotlib developers and
third-party library authors and focus on the low-level details.
...
aspects of the code and motivation.
...
needs.  The user documentation will also focus on how things should be
